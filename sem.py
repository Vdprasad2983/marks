...
def grade_point(n):
    if n=="A+":
        ...
        return 0
with open("C:\\Users\\ABC\\Desktop\\prasad files\\all semester results\\P3_Result of I B.Tech II Semester (R19-R20) RegularSupplementary Examinations, August-2022.csv","r") as f:
    x=csv.reader(f)
    data=list(x)
    ...
    sl.title("Ranking order in the I-II sem")
    ...
    sl.metric(label="pass percentage",value=(passper),delta=(passper-40.0))
    t=pd.DataFrame({"Roll Number":[s_l1[i][0] for i in range(len(s_l1))],"CGPA":[s_l1[i][1] for i in range(len(s_l1))],"Percentage":[s_l1[i][2] for i in range(len(s_l1))],"status":[s_l1[i][3] for i in range(len(s_l1))]})
    sl.table(t)
