...
with open("ecemarks_2.csv","r") as f:
    ...
    #c=l[144:216]
    #d=l[216:288]
    sl.set_page_config(page_title='ACOE',page_icon="acetlogo.jpeg")
    sl.image('university.png',width=400)
    sl.title("Marks of ACOE ECE in the III-II sem")
    
    selected=option_menu(menu_title=None,options=["profile","console data","custom"],
                         icons=["cast","book","pencil"],default_index=0,orientation="horizontal")
    col1,col2,col3=sl.columns(3)
    r=col1.text_input("enter your roll number*",max_chars=10)
    roll=r.upper()
    pre=col2.text_input("enter you total cgpa upto this result",value=0.0,max_chars=4)
    ...
    if selected=="custom":
        cus=list(sl.multiselect("select the numbers",options=l))
        l2=[i for i in s_l1 if i[0] in cus]
        s_l2=sorted(l2,key=lambda x: x[2],reverse=True)
        t=pd.DataFrame({"Roll Number":[s_l2[i][0] for i in range(len(s_l2))],"NAME":[s_l2[i][1] for i in range(len(s_l2))],
                        "SGPA":[s_l2[i][2] for i in range(len(s_l2))],"Percentage":[s_l2[i][3] for i in range(len(s_l2))],
                        "status":[s_l2[i][6] for i in range(len(s_l2))]})
        t = t.set_index([pd.Index([i for i in range(1,len(s_l2)+1)])])
        try:
            sl.table(t.style.map(color_df,subset=['status']))
        except ZeroDivisionError:
            sl.error("enter atleast one number")
    ...
    if(selected=="profile"):
        ...
        for i in s_l1:
            if i[0]==roll:
                ...
                if(i[6]=="pass"):
                    sl.success(f"{i[1]} passed in all subjects and not having any backlogs")
                else:
                    sl.error(f"{i[1]} failed in one or more subject(s)")
